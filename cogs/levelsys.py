...
bot_channel = 0#put the channel ID here. You can get the channel ID by heading over to the channel > right click > copy channel ID
...
mongowallecollection = cluster["mongowalledb"]["mongowallecollection"] #replace the database_name and the mongowallecollection from MongoDB here.
...
#gaining exp. It gains 1 exp but you can change it accordingly.
    ...
    async def on_message(self, message):
        if message.channel.id in talk_channels: #to check if it's in the right channel
            stats = mongowallecollection.find_one({"id":message.author.id}) #replace mongowallecollection with your collection's name
            if not message.author.bot: #to check that it isn't levelling the bot up.
                if stats is None: #to check if they're registered
                    ...
                    mongowallecollection.insert_one(newuser) #replace mongowallecollection with you collection's name; this is to insert the details into the database
                else: #means that they're registered
                    xp = stats["xp"] + 1 #increases xp by 1
                    mongowallecollection.update_one({"id":message.author.id}, {"$set":{"xp":xp}}) #replace mongowallecollection with your collection's name; is being ussed to update the databse
...
#to get rank
    ...
    async def rank(self, ctx):
        if ctx.channel.id == bot_channel: #to check if they're sending it in the right channel
            stats = mongowallecollection.find_one({"id" : ctx.author.id}) #replace mongowallecollection with your collection's name
            if stats is None: #checks if the user has send messages or not. If not then it send the message mentioned below
                ...
                rankings = mongowallecollection.find().sort("xp",-1) #replace mongowallecollection with your collection's name

...
class Moderation(commands.Cog):
    ...
    async def clear(self, ctx, amount=10):
        ...
        if amount > 101:
            ...
            msg = await ctx.send(f"✅ Cleared Messages `{amount-1} message! 🗑️`")
    ...
    async def modban(self, ctx, member:  Optional[Union[nextcord.Member, nextcord.User]], *, reason=None):
    ...
    async def starboard(self, ctx):
        try:
            ...
            embed = nextcord.Embed(title="Starboard Error!", color=nextcord.Color.red(), description=f"You don't have your starboard setup!, use `{PREFIX}starboard setup` to set it up!")
            embed.set_footer(text="P.S. Use `!help starboard` for more info!")
            return await ctx.send(embed=embed)
        embed = nextcord.Embed(title=f"{ctx.guild.name}'s Starboard Settings")
    ...
    async def setup(self, ctx):
        ...
        try:
            ...
            embed = nextcord.Embed(title="Starboard Error!", color=nextcord.Color.red(), description=f"The starboard is already setup for this server!\nUse `{PREFIX}starboard` to view the settings!\nDo you want to wipe all the data and start again?")
            view=nextcord.View()
            ...
            while r:
                try:
                    ...
                    if res.data['component_type'] != 2:
                        ...
                        embed=nextcord.Embed(title="Success!", color=nextcord.Color.green(), description="Let's start the setup!")
                        await ctx.send(embed=embed)
                        r=False
                        ...
                    elif res.data['custom_id'] == "False":
                        embed=nextcord.Embed(title="Cancelled!", description="**Nothing Happened**", color=nextcord.Color.red())
                        await ctx.send(embed=embed)
                        r=False
                        return
                except asyncio.TimeoutError:
                    return
        embed = nextcord.Embed(title="Let's start the Starboard setup!", description="Where should I post the starboard?")
        ...
        while retry:
            try:
                ...
                if response.content == "cancel":
                    retry=False
                    await ctx.send(embed=nextcord.Embed(color=nextcord.Color.red(), description="Cancelling..."))
                else:
                    try:
                        ...
                        embed=nextcord.Embed(title="Success!", color=nextcord.Color.green(), description=f"The starboard will be sent to {channel_to_post_in.mention}")
                        await ctx.send(embed=embed)
                        embed=nextcord.Embed(description="What should be the minimum star amount?")
                        await ctx.send(embed=embed)
                        re=True
                        while re:
                            try:
                                ...
                                if response.content == "cancel":
                                    re=False
                                    await ctx.send(embed=nextcord.Embed(description="Cancelling....", color=nextcord.Color.red()))
                                else:
                                    try:
                                        int(response.content)
                                        database[str(ctx.guild.id)]['starboard']['minimum stars'] = response.content
                                        await ctx.send(embed=nextcord.Embed(title="Success!",color=nextcord.Color.green(), description=f"Successfully set the minimum amount of stars to `{response.content}`"))
                                        re=False
                                        ...
                                        def check(i):
                                            return i.user.id == ctx.author.id
                                        embed=nextcord.Embed(title="Do you want to save the changes?")
                                        await ctx.send(embed=embed, view=view)
                                        r=True
                                        while r:
                                            try:
                                                ...
                                                if res.data['component_type'] != 2:
                                                    ...
                                                    embed=nextcord.Embed(title="Success!", color=nextcord.Color.green(), description="The changes were successfully saved!")
                                                    await ctx.send(embed=embed)
                                                    r=False
                                                elif res.data['custom_id'] == "False":
                                                    embed=nextcord.Embed(title="Cancelled!", description="The changes were not saved!", color=nextcord.Color.red())
                                                    await ctx.send(embed=embed)
                                                    r=False
                                            except asyncio.TimeoutError:
                                                embed=nextcord.Embed(description="Let's pretend that never happened!")
                                                return await ctx.send(embed=embed)
                                    except commands.BadArgument:
                                        embed=nextcord.Embed(title="Error!", description=f"Can't convert `{response.content}` to a number, try again.", color=nextcord.Color.red())
                                        await ctx.send(embed=embed)
                            except asyncio.TimeoutError:
                                embed=nextcord.Embed(description="Let's pretend that never happened!")
                                return await ctx.send(embed=embed)
                        ...
                        embed=nextcord.Embed(title="Error!", color=nextcord.Color.red(), description="I can't find that channel ;-;")
                        await ctx.send(embed=embed)
            except asyncio.TimeoutError:
                embed=nextcord.Embed(description="Let's pretend that never happened!")
                return await ctx.send(embed=embed)
    ...
    async def toggle(self, ctx):
        ...
        try:
            ...
            embed = nextcord.Embed(title="Starboard Error!", color=nextcord.Color.red(), description=f"You don't have your starboard setup!, use `{PREFIX}starboard setup` to set it up!")
            return await ctx.send(embed=embed)
        embed=nextcord.Embed(title=f"My Starboard Settings For {ctx.guild.name}", color=nextcord.Color.green() if guild_starboard_settings['on or off'] else nextcord.Color.red(), description=f"The starboard is `{'Enabled' if guild_starboard_settings['on or off'] else 'Disabled'}` for this server.")
        await ctx.send(embed=embed)
    ...
    async def on(self, ctx):
        ...
        if guild_starboard_settings['on or off']:
            return await ctx.send(embed=nextcord.Embed(title="Error!", description="The starboard for this server is already enabled!", color=nextcord.Color.red()))
        guild_starboard_settings['on or off'] = True
        write_database(data=database)
        await ctx.send(embed=nextcord.Embed(title="Success!", color=nextcord.Color.green(), description="The starboard has been `enabled` for this server!"))
    ...
    async def off(self, ctx):
        ...
        if not guild_starboard_settings['on or off']:
            return await ctx.send(embed=nextcord.Embed(title="Error!", description="The starboard for this server is already disabled!", color=nextcord.Color.red()))
        guild_starboard_settings['on or off'] = False
        write_database(data=database)
        await ctx.send(embed=nextcord.Embed(title="Success!", color=nextcord.Color.red(), description="The starboard has been `disabled` for this server!"))
    ...
    async def channel(self, ctx):
        ...
        await ctx.send(embed=nextcord.Embed(title=f"My Starboard Channel Settings For {ctx.guild.name}", color=nextcord.Color.green(), description=f"I post the starboard in <#{guild_starboard_settings['channel']}>"))
    ...
    async def change(ctx,*, channel=None):
        ...
        try:
            ...
            write_database(data=database)
            await ctx.send(embed=nextcord.Embed(title="Success!", color=nextcord.Color.green(), description=f"Successfully set the starboard channel to {channel_to_post_in.mention}(#{channel_to_post_in.name})"))
        except:
            await ctx.send(embed=nextcord.Embed(title="Error!", color=nextcord.Color.red(), description="I wasn't able to find that channel ;-;"))
    ...
    async def minimum_stars(self, ctx):
        ...
        try:
            ...
            embed = nextcord.Embed(title="Starboard Error!", color=nextcord.Color.red(), description=f"You don't have your starboard setup!, use `{PREFIX}starboard setup` to set it up!")
            return await ctx.send(embed=embed)
        await ctx.send(embed=nextcord.Embed(title=f"My Starboard Minimum Stars Settings For {ctx.guild.name}", color=nextcord.Color.green()))
    ...
    async def change(ctx, *, number):
        ...
        try:
            ...
            write_database(data=database)
            await ctx.send(embed=nextcord.Embed(title="Success!", color=nextcord.Color.green(), description=f"Successfully set the starboard minimum stars requirement to {number}"))
        except:
            await ctx.send(embed=nextcord.Embed(title="Error!", color=nextcord.Color.red(), description="I wasn't able to convert the number to an integer ;-;"))



    @staticmethod
    async def addwarn(ctx: commands.Context,reason,user):
        db = await aiosqlite.connect('db/warns.db')
        await asyncio.sleep(3)
        async with db.cursor() as cursor:
            await cursor.execute("CREATE TABLE IF NOT EXISTS warns(user INTEGER, reason TEST, time INTEGER, guild INTEGER)")
    ...
    @commands.command(name="warn",description="Warns a user")
    @commands.has_permissions(manage_messages=True)
    async def warn(self,ctx: commands.Context, member: nextcord.Member, *,reason: str="No Reason Provided"):

        em = nextcord.Embed(colour=0xff001e)
        em.add_field(name=f"⚠️ Warned {member.name}#{member.discriminator} for:",value=f"{reason}")
        await Moderation.addwarn(ctx=ctx,reason=reason,user=member)
        await ctx.send(embed=em)
    
    @commands.command(name="removewarn",description="Remove a users warning")
    @commands.has_permissions(manage_guild=True)
    async def removewarn(self,ctx: commands.Context, member: nextcord.Member):
        ...
        async with db.cursor() as cursor:
            ...
            if data:
                ...
                em = nextcord.Embed(title=f"Deleted {member.name}#{member.discriminator} warning's ✅",colour=0x00ff04)
                await ctx.send(embed=em)
            else:
                em = nextcord.Embed(title=f"No warnings found! of {member.name}#{member.discriminator}",colour=0xf6ff00)
                await ctx.send(embed=em)
        await db.commit()

    @commands.command(name="warns",description="Views a users warning")
    @commands.has_permissions(manage_messages=True)
    async def warns(self,ctx: commands.Context, member: nextcord.Member):
        ...
        async with db.cursor() as cursor:
            ...
            if data:
                em = nextcord.Embed(title=f"{member.name}'s Warning's",colour=0xff001e)
                warnnums = 0
                for table in data:
                    warnnums +=1
                    em.add_field(name=f"⚠️ Warning {warnnums}", value=f"Reason: {table[0]}  |  Date Issued: <t:{int(table[1])}:F>")
                await ctx.send(embed=em)
            else:
                em = nextcord.Embed(title=f"No warnings found! of {member.name}#{member.discriminator}",colour=0xf6ff00)
                await ctx.send(embed=em)

...
class voice(commands.Cog):
    ...
    @commands.command(name="voice-help")
    async def voicehelp(self, ctx):
        embed = nextcord.Embed(title="Help", description="",color=0x2b2d31

)
        embed.set_author(name=f"{ctx.guild.me.display_name}", icon_url=f"{ctx.guild.me.display_avatar.url}")
        embed.add_field(name=f'**Voice Commands**', value=f'**Lock your channel by using the following command:**\n\n`{PREFIX}voice lock`\n\n------------\n\n'
                        f'**Unlock your channel by using the following command:**\n\n`{PREFIX}voice unlock`\n\n------------\n\n'
                        f'**Change your channel name by using the following command:**\n\n`{PREFIX}voice name <name>`\n\n**Example:** `{PREFIX}voice name OogaBooga`\n\n------------\n\n'
                        f'**Change your channel limit by using the following command:**\n\n`{PREFIX}voice limit number`\n\n**Example:** `{PREFIX}voice limit 2`\n\n------------\n\n'
                        f'**Give users permission to join by using the following command:**\n\n`{PREFIX}voice permit @person`\n\n**Example:** `{PREFIX}voice permit @Ninja#0112`\n\n------------\n\n'
                        f'**Claim ownership of channel once the owner has left:**\n\n`{PREFIX}voice claim`\n\n**Example:** `{PREFIX}voice claim`\n\n------------\n\n'
                        f'**Remove permission and the user from your channel using the following command:**\n\n`{PREFIX}voice reject @person`\n\n**Example:** `{PREFIX}voice reject @Ninja#0112`\n\n', inline='false')
        embed.set_footer(text='Bot developed by Ninja#0112')
    ...
    async def setup(self, ctx):
        ...
        id = ctx.author.id
        if ctx.author.id == ctx.guild.owner_id or ctx.author.id == 410321996463996930:
            def check(m):
    ...
    async def setlimit(self, ctx, num):
        conn = sqlite3.connect('db/voice.db')
        c = conn.cursor()
        if ctx.author.id == ctx.guild.owner.id or ctx.author.id == 410321996463996930:
